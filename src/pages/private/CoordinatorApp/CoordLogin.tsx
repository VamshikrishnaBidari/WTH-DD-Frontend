...
const CoordLogin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // console.log("Checking coordinator and userRole:", coordinator, userRole);
    if (coordinator) {
      if (coordinator.role === "operator" || userRole === "operator") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await api.post("/operator/login", {
        email,
        password,
      });
      if (response.data.success) {
        ...
        try {
          ...
          if (getFullUser.data.success) {
            ...
            // console.log(
            //   "Full user details fetched successfully:",
            //   getFullUser.data.user,
            // );
          } else {
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6 md:p-8 space-y-6">
          ...
          {/* <button
            ...
            className="w-full flex items-center justify-center gap-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          >
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Don't have a coordinator account?{" "}
              <Link
                to="/contact"
