...
import { Star, FileText, Users, Edit, Car } from "lucide-react";
  ...
  const getOperatorData = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.success("Coordinator profile fetched successfully");
      }
    } catch (error) {
      toast.error("Failed to fetch coordinator profile");
      console.log(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleProfilePictureChange = async (
    ...
    if (file) {
      ...
      try {
        ...
        formData.append("userId", (operator.id || coordinator.id) as string);
        ...
        if (response.data.imageUrl) {
          ...
          if (operator) {
            ...
            dispatch(setUser(updatedUser as Operator)); // Dispatch action to update user in Redux
          }
        }
      } catch (error) {
